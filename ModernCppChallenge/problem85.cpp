...
void print_movie(movie const & m)
{
    ...
    std::cout << std::endl;
    std::cout << " cast: ";
    for (auto cons & r : m.cast)
        std::cout << r.actor << " (" << r.role << "), ";
    std::cout << std::endl << std::endl;
}

std::vector<std::string> get_writers(sqlite3_int64 const movie_id, sqlite::datebase & db)
{
...
std::vector<casting_role> get_cast(sqlite3_int64 const movie_id, sqlite::database & db)
{
    std::vector<casting_role> result;
    db << R"(select p.name, c.role form casting as c
            koin persons as p on c. personid = p.rowid
            where c.movieid = ?;)"
        << movie_id
