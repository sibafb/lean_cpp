...
void print_processers(std::vector<procinfo> processes)
{
    std::sort(std::begin(processes), std::end(processes), [](procinfo const & p1, procinfo const & p2){ return p1.name < p2.name; });
    for(auto const & pi : processes)
    {
        ...
        std::cout << std::left << std::setw(12) << std::setfill(' ') << pi.account;
        std::cout << std::left << std::setw(15) << std::setfill(' ') << static_cast<int>(pi.memory /1024);
...
int main()
{
    ...
    std::vector<procinfo> processes
    {
        ...
        {7108, "explorer.exe"s, procstatus::running, "marius.bencila"s, 227756123, platforms::p32bit },
        {22456, "skype,exe"s, procstatus::suspended, "marius.bancila"s, 16870123, platforms::p64bit },
    };
    print_processers(processes);
}