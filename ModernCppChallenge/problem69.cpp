...
protected:
    ...
    virtual int next_random(unsigned int const year, unsigned int const month, unsigned int const day) = 0;
...
public:
    std::string generate(
        sex_type const sex,
        unsigned int const year, unsigned int const month, unsigned int const day)
    {
        ...
        auto const sum = std::accumulate(std::begin(number), std::end(number), 0U,
...
protected:
    ...
    virtual int next_random(unsigned int const year, unsigned int const month, unsigned int const day) override
    {
        ...
        while(true)
        {
            auto const number = ud(eng);
            auto const pos = cache.find(number);
...
public:
    ...
    social_number_generator* get_generator(std::string const & country) const
    {
        auto const it = generators.find(country.c_str());
...
int main()
{
    ...
    auto sn3 = factory.get_generator("northeria")->generate(sex_type::female, 2017, 12, 25);
    ...
    auto ss3 = factory.get_generator("southeria")->generate(sex_type::female, 2017, 12, 25);
}